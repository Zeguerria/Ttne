    ...
    public static function restaurerSelection(array $ids)
    {
        ...
        try {
            ...
            foreach ($corbeilles as $corbeille) {
                ...
                /**
                 * --------------------------------------------------------------
                 * | RECUPERATION DU MODELE ORIGINAL
                 * --------------------------------------------------------------
                 */
                $record = $corbeille->getModelInstance();
                ...
                if (!$record) {
                    throw new Exception(
                        "Impossible de retrouver l'élément supprimé "
                        . "(corbeille #{$corbeille->id})."
                    );
                }
                 ...
                 * --------------------------------------------------------------
                 */
                if ($record->supprimer == 0) {

                    throw new Exception(
                        "L'élément de la corbeille #{$corbeille->id} "
                        . "est déjà actif."
                    );
                }
                ...
                /**
                 * --------------------------------------------------------------
                 * | SUPPRESSION DE L'ELEMENT DE LA CORBEILLE
                 * --------------------------------------------------------------
                 */
                $corbeille->delete();
